...
class BasePipeline(torch.nn.Module):
    ...
    # Original download_and_load_models function (commented out above) has OOM issues when loading models because it
    # tries to load every instance onto `cuda` instead of `cuda:RANK`. This is a fix from:
    # https://github.com/modelscope/DiffSynth-Studio/issues/1164#issuecomment-3743594631
    # which has not been merged into DiffSynth-Studio at this point.
    def download_and_load_models(self, model_configs: list[ModelConfig] = [], vram_limit: float = None):
        import os
        import torch
        ...
        def _dbg(tag):
            ...
            msg = f"[download_and_load_models:{tag}] RANK={rank} LOCAL_RANK={local_rank} WORLD_SIZE={world} " \
                f"self.device={getattr(self, 'device', None)} CUDA_VISIBLE_DEVICES={cvd}"
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    print(f"[download_and_load_models:{tag}] torch.cuda.current_device()={cur} name={name}", flush=True)
                    t = torch.empty(1, device="cuda")
                    print(f"[download_and_load_models:{tag}] probe tensor device={t.device}", flush=True)
                except Exception as e:
                    print(f"[download_and_load_models:{tag}] cuda debug failed: {e}", flush=True)

        _dbg("enter")

        model_pool = ModelPool()
        for i, model_config in enumerate(model_configs):
            print(f"[download_and_load_models] --- model_config[{i}] ---", flush=True)
            print(f"[download_and_load_models] path={getattr(model_config, 'path', None)}", flush=True)
            print(f"[download_and_load_models] model_id={getattr(model_config, 'model_id', None)} "
                f"origin_file_pattern={getattr(model_config, 'origin_file_pattern', None)}", flush=True)
            ...
            _dbg(f"after_auto_load_model[{i}]")
